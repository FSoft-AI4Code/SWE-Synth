"""
This script aim to:
1. Convert field from `text: str` to `messages: List[Dict[str, str]]` format
2. Create `eval_text` and `messages_eval` fields also, for generation
3. Filter out rows with more than `max_tokens` tokens
4. Prepare the dataset for training format
"""
...
def main(dataset_path, max_tokens, output_dir):
    ...
    # assert max_tokens == -1, "max_tokens is deprecated"
    # dataset = load_from_disk(dataset_path)["dev"]
    dataset = load_dataset("parquet", data_files={"dev": dataset_path}, split="dev")
    dataset = dataset.train_test_split(test_size=300, seed=42)
    dataset
    ...
    # drop rows with more than 56K tokens
    # num_max_tokens = 64000
    ...
    print(f"Max tokens in train: {max(formatted_dataset['train']['num_tokens'])}")
    print(f"Max tokens in test: {max(formatted_dataset['test']['num_tokens'])}")
    Path(output_dir).mkdir(parents=True, exist_ok=True)

    formatted_dataset.save_to_disk(f"{output_dir}/formatted_dataset")

    llama_factory_df = [{"messages": x} for x in formatted_dataset["train"]["messages"]]

    output_file = Path(f"{output_dir}/formatted_dataset_llama_factory_train.jsonl.zst")
    # with zstd.open(f'{output_dir}/formatted_dataset_llama_factory_train.json.zst', 'wt', encoding='utf-8') as f:
    #     json.dump(llama_factory_df, f)
    # decode with `zstdcat {output_dir}/formatted_dataset_14000_llama_factory.json.zst`
    with zstd.open(output_file, "wt", encoding="utf-8") as f:
        for line in llama_factory_df:
